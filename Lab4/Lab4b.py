            ...
            for i in range(len(self.stateCost)):
                for j in range(len(self.stateCost[i])):
                    ...
                    for a in self.actions[i][j]:
                        ...
                        for p in poss:
                            # Para cada direcao, verifica se ele bate na parede ou se move para a direcao desejada
                            # e soma o custo correspondente para cada caso com o custo da celula de destino
                            if p[0] == "U":
                                if i == 0:
                                    summing -= p[1]*-1
...
class Robot:
    ...
    def restart(self, world):
        '''
        Funcao define aleatoriamente a posicao inicial do robo
        :param world: tabuleiro contande os espacos vazios, Ouro, Wumpuns e o Poço
        '''
        tile = ""
        self.qtdRestart += 1
        while tile != " ":
